...
# bashblog-simple. even simpler than existing.
# (C) Rommel Sanchez-Verdejo <rsv-dev@cuevano.org>, 2023 and contributors
# https://github.com/rommelsv/bashblog
...
global_variables() {
    global_software_name="BashBlog-simple"
    global_software_version="2.11-rsv"

    # Blog title
    global_title="My Personal Blog"
    # The typical subtitle for each blog
    global_description="Statically Generated by bashblog-simple"
    ...
    # CC by-nc-nd is a good starting point, you can change this to
    # &copy;" for Copyright
    ...
    # Non blogpost files.
    # Bashblog will ignore these. Useful for static pages and custom content
    # Add them as a bash array, e.g. non_blogpost_files=("news.html" "test.html")
    non_blogpost_files=()

    # Declare your menu here. Key is the printed name, value is the file to
    # point to as for example:
...
is_boilerplate_file() {
    ...
    case $name in
    ( "$index_file" | "$archive_index" | "$tags_index" | "$footer_file"  \
    | "$header_file" | "$prefix_tags"*  |  \
    ".title.html" |  ".footer.html" | ".header.html" | ".menu.html" )
        return 0 ;;
    ( * ) # Check for excluded
...
create_html_page() {
    ...
    # html, head
    {
        ...
        if [[ $index == no ]]; then
            ...
            echo "<h3><a class=\"ablack\" href=\"$global_url/$file_url\">"
    ...
    while IFS='' read -r line; do
        if [[ -z $title ]]; then
            ...
            if [[ -n $3 ]] && [[ ! -d $3 ]]; then
                ...
                [[ -d $3 ]] && filename=$3/$filename
                echo "Infooooo " $filename
...
# also the drafts
write_entry() {
    test_markdown && fmt=md
    f=$2

    path_prefix=$(LC_ALL=$date_locale date +%Y/%m)
    mkdir -p $path_prefix

    if [[ -n $f ]]; then
        TMPFILE=$path_prefix/$f
        if [[ ! -f $TMPFILE ]]; then
        ...
        TMPFILE=$path_prefix/.entry-$RANDOM.$fmt
...
EOF
    ...
    while [[ $post_status != "p" && $post_status != "P" ]]; do
        ...
        if [[ $fmt == md ]]; then
            ...
            edit_timestamp=$(LC_ALL=C date  "+${date_format_full}")
            parse_file "$html_from_md" "${edit_timestamp}" "${path_prefix}"
            rm "$html_from_md"
        fi
...
rebuild_index() {
    ...
    # Create the content file
    {
        ...
        while IFS='' read -r i; do
            ...
            echo -n "." 1>&3
            n=$(( n + 1 ))
        done < <(find . -iname '*.html' -printf "%T+ %p\n" | sort -r |cut -d ' ' -f 2)
...
rebuild_tags() {
    ...
    if (($# < 2)); then
        # will process all files and tags
            files=$(find . -iname '*.html' -printf "%T+ %p\n" | sort -r |cut -d ' ' -f 2)
        all_tags=yes
    else
    ...
    while IFS='' read -r i; do
        ...
        echo -n "."
        if [[ -n $cut_do ]]; then
            link_url="$global_url/$i"
            get_html_file_content 'entry' 'entry' 'cut' <"$i" | awk "/$cut_line/ { print \"<p class=\\\"readmore\\\"><a href=\\\"$link_url\\\">$template_read_more</a></p>\" ; next } 1"
        else
            get_html_file_content 'entry' 'entry' <"$i"
        fi >"$tmpfile"
...
create_includes() {
    ...
    if [[ ${#global_main_menu[@]} > 0 ]]; then
        {
            echo "<nav id='global_main_menu' class='navigation-menu'>"
            echo "<label class='hamburger-icon' aria-label='Open navigation menu' for='menu-toggle'>&#9776;</label>"
            echo "<input type='checkbox' id='menu-toggle'/><ul class='main-navigation'>"
    ...
    if [[ -f $header_file ]]; then cp "$header_file" .header.html
        ...
        printf '<link rel="stylesheet" href="%s" type="text/css">\n' "$global_url/${css_include[@]}"
        if [[ -z $global_feedburner ]]; then
            echo "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"$template_subscribe_browser_button\" href=\"$global_url/$blog_feed\">"
        else
            echo "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"$template_subscribe_browser_button\" href=\"$global_url/$global_feedburner\">"
        fi
        } > ".header.html"
    fi
    ...
    if [[ -f $footer_file ]]; then cp "$footer_file" .footer.html
        ...
        echo "<div id=\"footer\">Copyright &copy; ${copyright_year} ${global_author}"
        echo "<br>${global_license}<br><a href=\"${global_author_url}\">${global_author}</a>"
        echo "&mdash; <a href=\"mailto:${protected_mail}\">${protected_mail}</a><br>"
        echo "Generated with <a href="https://www.github.com/rommelsv/bashblog">bashblog-simple</a>"
        echo " - (B)e (A)wesome & (S)imple (H)omie</div>"
        } >> ".footer.html"
    fi
}
...
rebuild_all_entries() {
    ...
    for i in $(find . -iname '*.html' -not -iname '.*.html' -printf "%T+ %p\n" | sort -r | cut -d ' ' -f 2); do
        is_boilerplate_file "$i" && continue;
        [[ "$(dirname $i)" == "." ]] && migrate_folder="true"
        ...
        dir_prefix=$(LC_ALL=$date_locale date -r "$i" +%Y/%m)
        [[ "${migrate_folder}" == "true" ]] && [[ ! -d ${dir_prefix}  ]] && mkdir -p ${dir_prefix}
        if [[ "$(dirname $i)" != "./${dir_prefix}" ]] ;then
            target_i=$dir_prefix/$i
            fname=$(basename $i .html)
            rm $i; mv -v $fname.md $dir_prefix/$fname.md
        else
            target_i=${i}
        fi
        mv "$i.rebuilt" "$target_i"
        chmod 644 "$target_i"
        touch -t "$timestamp" "$target_i"
        rm $contentfile
    done
    echo ""
}
...
usage() {
    ...
    echo ""
    echo "Commands:"
    echo "    post [filename]         inserts a new blog post, or the filename of a draft to continue editing it"
    echo "                            it tries to use markdown by default."
...
reset() {
    ...
    if [[ $line == "Yes, I am!" ]]; then
        for each in '*.html' '*.css' '*.rss'; do
            $(find . -iname "${each}" -exec rm -f {} +) &> /dev/null
        done
        echo
...
do_main() {
    ...
    create_includes
    [[ $1 == post ]] && write_entry "$@"
    # Fixme: twice to rewrite the "link" at title when moved to fs directories
    [[ $1 == rebuild ]] && rebuild_all_entries && rebuild_tags && rebuild_all_entries && rebuild_tags
